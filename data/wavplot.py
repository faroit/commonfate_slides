...
def wavplot(
    ...
    frame_length = int(math.ceil(num_samples / float(num_bars)))
    data.resize((num_bars, frame_length))

    x_mean, x_max = zip(*[
        (np.sqrt(np.mean(block**2)), np.max(abs(block))) for block in
        data
    ])
